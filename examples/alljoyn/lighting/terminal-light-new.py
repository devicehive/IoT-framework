...
DEVICEID = 'c50ded5d5dfc4de28eb296e921d9a6e0'
...
## In percentage, 0 means 0. uint32_max-1 means 100. */
    ...
    def GetAboutData(self, languageTag):
        ...
        return {
            'AppId': dbus.ByteArray(bytes.fromhex(DEVICEID)),
            'DefaultLanguage': 'en',
...
class Lamp():
  def __init__(self, busname, name):
    ...
    self._container = core.BusContainer(busname, DBUS_BUS_PATH + '/' + DEVICEID)


    self._services = [
       core.AboutService(self._container, about_props),
       core.ConfigService(self._container, self.name),
       LampService(self._container.relative("/org/allseen/LSF/Lamp"))
    ]
  ...
  def publish(self, bridge):      
    ...
    bridge.AddService(self._container.bus.get_name(), self._container.relative('').rstrip('/'), DBUS_BUS_NAME,
      # ignore_reply=True
