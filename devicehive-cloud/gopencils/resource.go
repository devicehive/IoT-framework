...
type Resource struct {
	...
	requestOut chan *http.Request
}

func (r *Resource) SetRequestOut(out chan *http.Request) {
	r.requestOut = out
}

func (r *Resource) PopRequest() *http.Request {
	if r.requestOut != nil {
		select {
		case req := <-r.requestOut:
			return req
		default:
			return nil
		}
...
func (r *Resource) do(method string) (*Resource, error) {
	...
	r.PopRequest()
	if r.requestOut == nil {
		r.requestOut = make(chan *http.Request, 1)
	}
